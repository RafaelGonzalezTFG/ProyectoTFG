...
<head>
    ...
    <style>
        #sidebarMenu {
            margin-top: 70px;
            /* Ajusta este valor según la altura que necesites */
        }
    </style>
</head>
        ...
        <div class="row">
            ...
            <nav id="sidebarMenu" class="col-md-3 col-lg-2 d-md-block bg-light sidebar">
                <div class="position-sticky">
                    <ul class="nav flex-column">
                        ...
                        <li class="nav-item">
                            <a class="nav-link  active" href="../controller/mostrar_usuario_controlador.php">
            ...
            <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4 mt-navbar">
                ...
                <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
                    <h1 class="h2">Gestión de los Usuarios</h1>
                </div>

                <div class="w-100 d-flex justify-content-start mb-2">
                    <form method="POST" action="../controller/redireccionador_usuarios.php">
                        <button class="btn btn-warning" name="insertar" value="true">Insertar un nuevo Usuario</button>
                    </form>
                </div>
                        ...
                        <tbody>
                            ...
                            if (!empty($datosCuentas)) {
                                foreach ($datosCuentas as $cuenta) {
                                    echo "<tr>\n";
                                    echo "<td>" . htmlspecialchars($cuenta["Nombre"]) . "</td>\n";
                                    echo "<td>" . htmlspecialchars($cuenta["Apellido"]) . "</td>\n";
                                    echo "<td>" . htmlspecialchars($cuenta["Telefono"]) . "</td>\n";
                                    echo "<td>" . htmlspecialchars($cuenta["Correo"]) . "</td>\n";
                                    echo "<td>" . htmlspecialchars($cuenta["Rol"]) . "</td>\n";
                                    echo "<td>" . htmlspecialchars($cuenta["Estado"]) . "</td>\n";
                                    //Comienzo Botones
                                    echo "<td class='d-flex justify-content-around'>\n";
                                    //Boton Editar
                                    echo "<form method='POST' action='../controller/redireccionador_usuarios.php' class='d-inline'>\n";
                                    echo "<input type='hidden' name='idCuentas' value='" . $cuenta["idCuentas"] . "' />\n";
                                    echo "<button name='modificar' value='true' class='btn btn-success btn-sm'>Modificar</button>\n";
                                    echo "</form>\n";
                                    //Boton Eliminar
                                    echo "<form method='POST' action='../controller/borrar_usuarios_controlador.php' class='d-inline'>\n";
                                    echo "<input type='hidden' name='idCuentas' value='" . $cuenta["idCuentas"] . "' />\n";
